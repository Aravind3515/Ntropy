...
finaldata.to_hdf('C:\\Users\\aravi\\Documents\\Dvara-Internship\\Crop Yield Estimation\\Dataset\\finalmodel9.h5', key='df', mode='w')

#%%
FileDuration=r'C:\Users\aravi\Documents\Dvara-Internship\Crop Yield Estimation\Dataset\DurationMissing.csv'
df_duration=pd.DataFrame(duration_length)
df_duration.to_csv(FileDuration)

#%%

b=[]
for i in finaldata.columns:
    if finaldata[i].isnull().values.any():
        b.append(i)
        print(i)
        print(finaldata[i].isnull().sum())
        
#%%

index_=[i for i in range(0,705)]
X.index=index_

#%%

last=pd.DataFrame(columns=b)

values={}

check_for_nan = X['RainFall_Median8'].isnull()
check_for_nan = pd.DataFrame(check_for_nan)

for i in b:
    check_for_nan = X[i].isnull()
    check_for_nan = pd.DataFrame(check_for_nan)
    check_for_nan.index=index_
    for j in range(0, len(check_for_nan)):
        if check_for_nan[i][j] == True:
            #last[i].append(X['Plot_ID'][j])
            #values=values.append({i:X['Plot_ID'][j]}
            values[i]=X['Plot_ID'][j]
            last=last.append(values,ignore_index=True)
            
    
last[i].append(X['Plot_ID'][i])    
    

for i in range(0, len(check_for_nan)):
    if check_for_nan['RainFall_Median8'][i] == True:
        # print(i)
        print(X['Plot_ID'][i])
...
last[~last['NDVI_Max9'].isnull()]['NDVI_Max9']

last[~last['NDVI_Max9'].isnull()]['NDVI_Max9'].unique()
